...
1. `xcodegen generate` to produce `Kaysonic.xcodeproj` from [project.yml](project.yml).
2. Open in Xcode and run, or `xcodebuild -scheme Kaysonic -destination 'platform=iOS Simulator,name=iPhone 17 Pro' build`.
...
Layout: [KaysonicKit](Sources/KaysonicKit) is the platform-agnostic core (YXC client, discovery, preset engine, Spotify, config store); [App/](App) holds the iOS app, Watch app, and shared model.
